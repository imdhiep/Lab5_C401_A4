...
def crawl_catalog(
    ...
    db_connection = None
    if persist_to_db:
        run_note = f"target_facility={target_facility_title or 'ALL'}"
        run_id = begin_crawl_run(BOOKING_URL, note=run_note)
        db_connection = get_connection()
        clear_catalog_tables(db_connection)
        if monitor:
            monitor.log("Đã xóa dữ liệu catalog cũ, bắt đầu ghi dần vào SQLite")
    ...
    try:
        ...
        for facility_index, facility in enumerate(facilities, start=1):
            if persist_to_db and db_connection:
                _upsert_facility(db_connection, facility)
            if monitor:
                monitor.set_context(
            ...
            for specialty in specialty_items:
                ...
                if specialty_key not in specialty_seen:
                    ...
                    specialties.append(specialty_record)
                    specialty_seen.add(specialty_key)
                    if persist_to_db and db_connection:
                        _upsert_specialty(db_connection, specialty_record)
                    if monitor:
                        monitor.set_counts(specialty_count=len(specialties))
                    if run_id:
                        update_crawl_run(run_id, specialty_count=len(specialties))
                ...
                for doctor in doctor_items:
                    ...
                    if doctor_key not in doctor_seen:
                        ...
                        if persist_to_db and db_connection:
                            _upsert_doctor(db_connection, doctor_record)
                        if monitor:
                            monitor.set_counts(doctor_count=len(doctors))
                        if run_id:
                            update_crawl_run(run_id, doctor_count=len(doctors))
                    ...
                    for offset in range(days_ahead):
                        ...
                        for slot in slot_items:
                            ...
                            if persist_to_db and db_connection:
                                _upsert_slot(db_connection, slot_record)
                            if monitor:
                                monitor.set_counts(slot_count=len(slots))
                            if run_id:
                                update_crawl_run(run_id, slot_count=len(slots))
...
def _upsert_facility(connection, facility: dict[str, Any]) -> None:
    connection.execute(
        """
        INSERT OR REPLACE INTO facilities (id, slug, title, address, city, latitude, longitude)
        VALUES (:id, :slug, :title, :address, :city, :latitude, :longitude)
        """,
        facility,
    )
...
def _upsert_specialty(connection, specialty: dict[str, Any]) -> None:
    connection.execute(
        """
        INSERT OR REPLACE INTO specialties (id, facility_id, title, ab_booking_enabled)
        VALUES (:id, :facility_id, :title, :ab_booking_enabled)
        """,
        specialty,
    )
...
def _upsert_doctor(connection, doctor: dict[str, Any]) -> None:
    connection.execute(
        """
...
def _upsert_slot(connection, slot: dict[str, Any]) -> None:
    connection.execute(
        """
